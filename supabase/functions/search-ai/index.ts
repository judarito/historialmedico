...
async function expandQuery(query: string, memberContext?: string): Promise<DeepSeekExpansion> {
  if (!DEEPSEEK_API_KEY) {
...
- Si se proveen datos del paciente, prioriza términos que realmente aparecen en su historial
...
Responde ÚNICAMENTE con JSON válido.`;
  ...
  const contextBlock = memberContext?.trim()
    ? `\nDatos del historial del paciente (usa esto para afinar los términos):\n${memberContext}\n`
    : "";

  const userPrompt = `Busca en historial médico: "${query}"${contextBlock}
...
serve(async (req) => {
  ...
  try {
    const { query, limit = 30, memberContext } = await req.json();
    ...
    // 1. Expandir query con DeepSeek (con contexto del paciente si se provee)
    const expansion = await expandQuery(query.trim(), memberContext);

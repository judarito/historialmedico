-- ============================================================
-- 036 -- Limpiar informacion clinica de familiares conservando su ficha
--
-- 1. Borra visitas, adjuntos, medicamentos, horarios, examenes
--    y recordatorios de uno o varios familiares.
-- 2. Conserva la ficha del familiar en family_members.
-- 3. Devuelve conteos y file_paths para limpieza posterior de Storage si hace falta.
-- ============================================================
...
BEGIN
  ...
  IF NOT user_belongs_to_tenant(v_member.tenant_id) THEN
...
CREATE OR REPLACE FUNCTION public.clear_family_members_medical_data(
...
BEGIN
  IF COALESCE(array_length(p_family_member_ids, 1), 0) = 0 THEN
    RETURN jsonb_build_object(
      'success', TRUE,
      'results', v_results
    );
  ...
  FOREACH v_family_member_id IN ARRAY p_family_member_ids LOOP
    v_results := v_results || jsonb_build_array(
      public.clear_family_member_medical_data(v_family_member_id)
    );
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.clear_family_members_medical_data(UUID[]) TO authenticated;
